<?php echo $this->render('_componentes/_layout.php');
	...
	$corpo = $this->url(array('module' => 'espaco-aberto', 'controller' => 'perfil','action' => 'exibir', 'usuario' => '' ), null, true);
...
<div class="promocao">
	<div class="promocao-interna">
		...
		<?php echo $this->ShowGoBack(false,$this->baseUrl()."/espaco-aberto/agenda/listar/".$tipoPerfil);?>

		<h2><?php echo $this->escape($this->objeto['evento']);?></h2>
		...
		<strong>Colegas convidados:</strong>
		<ul>
		<?php
			foreach($this->marcacaoConvidado as $marcacao):
				$usuario = $boUsuario->obtemNomeUsuario($marcacao['idUsuario'], null);
				echo '<li><a contenteditable="false" class="custom" href="'.$corpo.$marcacao['idUsuario'].'" tabindex="-1" >'.$usuario[0]['nome'].'</a></li>';
			endforeach;
		?>
		</ul>
		<br/>
		<strong>Colegas confirmados:</strong>
		<?php
			foreach($this->marcacaoConfirmado as $marcacao):
				$usuario = $boUsuario->obtemNomeUsuario($marcacao['idUsuario'], null);
				echo '<a contenteditable="false" class="custom" href="'.$corpo.$marcacao['idUsuario'].'" tabindex="-1" >'.$usuario[0]['nome'].'</a>&nbsp;';
			endforeach;
		?>
	</div>
