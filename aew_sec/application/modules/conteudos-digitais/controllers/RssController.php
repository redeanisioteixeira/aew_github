<?php
class ConteudosDigitais_RssController extends Zend_Controller_Action {
    private $feed = null;
    ...
    /**
     * Lista os conteúdos mais recentes
     */
    ...
    public function maisrecentesAction() 
    {
        ...
        $options["where"]["conteudodigitalcategoria.idcanal != ? OR conteudodigitalcategoria.idcanal IS NULL"] = 1;
    ...
    /**
     * Cria os feed RSS 
     * @param BO Aew_Model_Bo_ConteudoDigital()
     ...
     * @param string $opcao
     * @param int $type
     * @return array configuração do FEED
     */
    ...
    private function fetchEntries($data)
    {
        foreach($data as $record)
        {
            ...
            if ($record->getFormato() || $record->getFormatoDownload())
            {
            ...
            $categories = array(
                                array("term" => ($conteudoTipo->getId() ? $conteudoTipo->getId() : ''), "label" => "type_id"),
                                array("term" => ($conteudoTipo->getNome() ? $conteudoTipo->getNome() : ''), "label" => "type"),
