 ...
 * @version    $Id: Select.php 1 2016-05-25 16:47:26Z admin $
 */
...
class Zend_Db_Select
{
    ...
    const SQL_ON         = 'ON';
    const SQL_ASC        = 'ASC';
    const SQL_ASC_NULLS_LAST  = 'ASC NULLS LAST';
    const SQL_DESC       = 'DESC';
    const SQL_DESC_NULLS_LAST  = 'DESC NULLS LAST';
    ...
    public function order($spec)
    {
        ...
        foreach ($spec as $val) {
            ...
            if ($val instanceof Zend_Db_Expr) {
                ...
                if (preg_match('/(.*\W)(' . self::SQL_ASC_NULLS_LAST . '|' . self::SQL_DESC_NULLS_LAST . '|' . self::SQL_ASC . '|' . self::SQL_DESC . ')\b/si', $val, $matches)) {
                    $val = trim($matches[1]);
                    $direction = $matches[2];
                }
                if (preg_match('/\(.*\)/', $val)) {
