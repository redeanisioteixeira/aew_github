...
class Sec_Bitly
{
	...
	public function bitly_v3_shorten($longUrl, $domain = '', $x_login = '', $x_apiKey = '') {
	  $result = array();
	  $url = self::bitly_oauth_api . "shorten?login=". self::bitlyLogin . "&apiKey=" . self::bitlyKey . "&longUrl=" . urlencode($longUrl);
	  if ($domain != '') {
		 $url .= "&domain=" . $domain;
	  }
	  if ($x_login != '' && $x_apiKey != '') {
	...
	public function bitly_v3_expand($data) {
	  ...
	  $url = self::bitly_api . "expand?login=" . self::bitlyLogin . "&apiKey=" . self::bitlyKey . "&format=json&hash=" . $data;
      $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_v3_validate($x_login, $x_apiKey) {
	  $result = 0;
	  $url = self::bitly_api . "validate?login=" . self::bitlyLogin . "&apiKey=" . self::bitlyKey . "&format=json&x_login=" . $x_login . "&x_apiKey=" . $x_apiKey;
	  $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_v3_clicks($data) {
	  ...
	  $url = self::bitly_api . "clicks?login=" . self::bitlyLogin . "&apiKey=" . self::bitlyKey . "&format=json&hash=" . $data;
	  $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_v3_referrers($data) {
	  ...
	  $tmp = array_reverse($tmp);
	  $data = $tmp[0];
	  $url = self::bitly_api . "referrers?login=" . self::bitlyLogin . "&apiKey=" . self::bitlyKey . "&format=json&hash=" . $data;
	  $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_v3_countries($data) {
	  ...
	  $tmp = array_reverse($tmp);
	  $data = $tmp[0];
	  $url = self::bitly_api . "countries?login=" . self::bitlyLogin . "&apiKey=" . self::bitlyKey . "&format=json&hash=" . $data;
	  $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_v3_clicks_by_minute($data) {
	  ...
	  $url = self::bitly_api . "clicks_by_minute?login=" . self::bitlyLogin . "&apiKey=" . self::bitlyKey . "&format=json&hash=" . $data;
	  $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_v3_clicks_by_day($data, $days = 7) {
	  ...
	  $url = self::bitly_api . "clicks_by_day?login=" . self::bitlyLogin . "&apiKey=" . self::bitlyKey . "&format=json&days=" . $days . "&hash=" . $data;
	  $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_v3_bitly_pro_domain($domain) {
	  $result = array();
	  $url = self::bitly_api . "bitly_pro_domain?login=" . self::bitlyLogin . "&apiKey=" . self::bitlyKey . "&format=json&domain=" . $domain;
	  $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_v3_lookup($data) {
	  ...
	  $url = self::bitly_oauth_api . "lookup?url=" . $data . "&access_token=" . self::bitlyKey;
	  $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_v3_authenticate($x_login, $x_password) {
	  $result = array();
	  $url = self::bitly_api . "authenticate";
	  $params = array();
	  $params['login'] = self::bitlyLogin;
	  $params['apiKey'] = self::bitlyKey;
	  $params['format'] = "json";
	...
	public function bitly_v3_info($data) {
	  ...
	  // make the call to expand
	  $url = self::bitly_api . "info?login=" . self::bitlyLogin . "&apiKey=" . self::bitlyKey . "&format=json&hash=" . $data;
	  $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_oauth_access_token($code, $redirect) {
	  ...
	  $params = array();
	  $params['client_id'] = self::bitly_clientid;
	  $params['client_secret'] = self::bitly_secret;
	  $params['code'] = $code;
	...
	public function bitly_v3_user_clicks($access_token, $days = 7) {
	  ...
	  $results = array();
	  $url = self::bitly_oauth_api . "user/clicks?access_token=" . $access_token . "&days=" . $days;
	  $output = json_decode($this->bitly_get_curl($url));
	  if (isset($output->{'data'}->{'clicks'})) {
	...
	public function bitly_v3_user_referrers($access_token, $days = 7) {
	  ...
	  $results = array();
	  $url = self::bitly_oauth_api . "user/referrers?access_token=" . $access_token . "&days=" . $days;
	  $output = json_decode($this->bitly_get_curl($url));
	  if (isset($output->{'data'}->{'referrers'})) {
		 $results['days'] = $output->{'data'}->{'days'};
		 $results['referrers'] = array();
	...
	public function bitly_v3_user_countries($access_token, $days = 7) {
	  ...
	  $results = array();
	  $url = self::bitly_oauth_api . "user/countries?access_token=" . $access_token . "&days=" . $days;
	  $output = json_decode($this->bitly_get_curl($url));
	  if (isset($output->{'data'}->{'countries'})) {
		 $results['days'] = $output->{'data'}->{'days'};
		 $results['countries'] = array();
	...
	public function bitly_v3_user_realtime_links($access_token) {
	  ...
	  $results = array();
	  $url = self::bitly_oauth_api . "user/realtime_links?format=json&access_token=" . $access_token;
	  $output = json_decode($this->bitly_get_curl($url));
	 ...
	 */
	public function bitly_v3_user_link_history($access_token) {
	  // $results = bitly_v3_user_link_history('BITLY_SUPPLIED_ACCESS_TOKEN');
	  $results = array();
	  $url = self::bitly_oauth_api . "user/link_history?format=json&access_token=" . $access_token;
	  $output = json_decode($this->bitly_get_curl($url));
	...
	public function bitly_v3_highvalue($access_token, $limit = 5) {
	  $results = array();
	  $url = self::bitly_oauth_api . "highvalue?access_token=" . $access_token . "&limit=" . $limit;
	  $output = json_decode($this->bitly_get_curl($url));
	 ...
	 * @see http://dev.bitly.com/data_apis.html
	 */
	public function bitly_v3_search($access_token, $query, $limit = 10, $offset = 0, $domain = '', $lang = '', $cities = '', $fields = array()) {
	  $results = array();
	  $url = self::bitly_oauth_api . "search?access_token=" . $access_token . "&query=" . urlencode($query) . "&limit=" . $limit . "&offset=" . $offset;
	  if ($domain != '') {
	 ...
	 * @see http://dev.bitly.com/data_apis.html
	 */
	public function bitly_v3_realtime_bursting_phrases($access_token) {
	  $results = array();
	  $url = self::bitly_oauth_api . "realtime/bursting_phrases?access_token=" . $access_token;
	  $output = json_decode($this->bitly_get_curl($url));
	 ...
	 * @see http://dev.bitly.com/data_apis.html
	 */
	public function bitly_v3_realtime_hot_phrases($access_token) {
	  $results = array();
	  $url = self::bitly_oauth_api . "realtime/bursting_phrases?access_token=" . $access_token;
	  $output = json_decode($this->bitly_get_curl($url));
	  if (isset($output->{'data'}->{'phrases'})) {
	 ...
	 * @see http://dev.bitly.com/data_apis.html
	 */
	public function bitly_v3_realtime_clickrate($access_token, $phrase) {
	  $results = array();
	  $url = self::bitly_oauth_api . "realtime/clickrate?access_token=" . $access_token . "&phrase=" . urlencode($phrase);
	  $output = json_decode($this->bitly_get_curl($url));
	 ...
	 * @see http://dev.bitly.com/data_apis.html
	 */
	public function bitly_v3_link_info($access_token, $link) {
	  $results = array();
	  $url = self::bitly_oauth_api . "link/info?access_token=" . $access_token . "&link=" . urlencode($link);
	  $output = json_decode($this->bitly_get_curl($url));
	 ...
	 * @see http://dev.bitly.com/data_apis.html
	 */
	public function bitly_v3_link_content($access_token, $link) {
	  $results = array();
	  $url = self::bitly_oauth_api . "link/content?access_token=" . $access_token . "&link=" . urlencode($link);
	  $output = json_decode($this->bitly_get_curl($url));
	 ...
	 * @see http://dev.bitly.com/data_apis.html
	 */
	public function bitly_v3_link_category($access_token, $link) {
	  $results = array();
	  $url = self::bitly_oauth_api . "link/category?access_token=" . $access_token . "&link=" . urlencode($link);
	  $output = json_decode($this->bitly_get_curl($url));
	 ...
	 * @see http://dev.bitly.com/data_apis.html
	 */
	public function bitly_v3_link_social($access_token, $link) {
	  $results = array();
	  $url = self::bitly_oauth_api . "link/social?access_token=" . $access_token . "&link=" . urlencode($link);
	  $output = json_decode($this->bitly_get_curl($url));
	 ...
	 * @see http://dev.bitly.com/data_apis.html
	 */
	public function bitly_v3_link_location($access_token, $link) {
	  $results = array();
	  $url = self::bitly_oauth_api . "link/location?access_token=" . $access_token . "&link=" . urlencode($link);
	  $output = json_decode($this->bitly_get_curl($url));
	 ...
	 * @see http://dev.bitly.com/data_apis.html
	 */
	public function bitly_v3_link_language($access_token, $link) {
	  $results = array();
	  $url = self::bitly_oauth_api . "link/language?access_token=" . $access_token . "&link=" . urlencode($link);
	  $output = json_decode($this->bitly_get_curl($url));
